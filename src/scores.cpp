...
void Scoreboard::printStats(int boardsize) {
  if(language == 1){
    ...
    constexpr auto any_key_exit_text = "Press any key to go back to menu: ";
    constexpr auto sp = "  ";
    ...
    constexpr auto header_border_text = "┌────────────────┬─────────────┐";
    constexpr auto footer_border_text = "└────────────────┴─────────────┘";
    const auto stats_attributes_text = {"최고 점수", "게임 횟수",
    "성공 횟수", "전체 이동",
    "전체 시간"};
...
void Scoreboard::save() {
  ...
  constexpr auto score_saved_text_kor = "점수가 저장되었습니다!";
  constexpr auto sp = "  ";
  ...
  if(language == 1){
    ...
    score_saved_richtext << green << bold_on << sp << score_saved_text_kor << bold_off
    << def << "\n";
  }
